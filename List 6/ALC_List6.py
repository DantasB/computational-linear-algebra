...
def first_order_edo_solver(diferential_function, t0, tf, delta, start_condition, control):
    ...
    for i in range(steps):
        ...
        if(control == 0):
            x_incognita.append(x_incognita[i] + delta * diferential_function(t_incognita[i], x_incognita[i]))
        elif(control == 1):
            K1 = diferential_function(t_incognita[i], x_incognita[i])
            K2 = diferential_function(t_incognita[i] + delta, x_incognita[i] + delta * K1)
            x_incognita.append(x_incognita[i] + delta / 2 * (K1 + K2))
        else:
            K1 = diferential_function(t_incognita[i], x_incognita[i])
            K2 = diferential_function(t_incognita[i] + delta / 2, x_incognita[i] + delta / 2 * K1)
            K3 = diferential_function(t_incognita[i] + delta / 2, x_incognita[i] + delta / 2 * K2)
            K4 = diferential_function(t_incognita[i] + delta, x_incognita[i] + delta * K3)
...
def second_order_edo_solver(second_order_diferential_function, t0, tf, delta, x0, derivate_x0, control=True):
    ...
    for i in range(steps):
        ...
        if(control):
            x2_lines = second_order_diferential_function(t_incognita[i], x_incognita[i], x_actual_line)
            x_incognita.append(x_incognita[i] + x_actual_line * delta + x2_lines * delta * delta / 2)
            x_actual_line = x_actual_line + x2_lines * delta

        else:
            K1 = delta / 2  * second_order_diferential_function(t_incognita[i], x_incognita[i], x_actual_line)
            Q  = delta / 2 * (x_actual_line + K1 / 2)
            K2 = delta / 2 * second_order_diferential_function(t_incognita[i] + delta / 2, x_incognita[i] + Q, x_actual_line + K1)
            K3 = delta / 2 * second_order_diferential_function(t_incognita[i] + delta / 2, x_incognita[i] + Q, x_actual_line + K2)
            L  = delta * (x_actual_line + K3)
            K4 = delta/ 2 * second_order_diferential_function(t_incognita[i] + delta, x_incognita[i] + L, x_actual_line + 2*K3)
